...
Idempotent: re-running on already-patched file is a no-op.

Usage:
  python3 apply-patch-fg.py [/path/to/extension.js]
...
def main():
    path = sys.argv[1] if len(sys.argv) > 1 else find_default_extension()
    ...
    # Detect state by looking at the F-site-2 / F.2 / G.1 / G.2 patched markers
    patched_markers = [
    ...
    if all(s.count(m) >= 1 for m in patched_markers):
        print("All splices already applied. Nothing to do.")
        return
