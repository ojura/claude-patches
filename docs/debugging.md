...
This loop is the general method; the worked example below uses the claude-patches
Patch K markers (`NEEDLE="PFGK DEMO seam"`). The fixture generator (`gen_demo`),
the marker-specific gotchas, and the diagnostic case studies live in
...
`PFGK DEMO seam` fixture exists in an open-workspace project (generate it with `gen_demo`; see `patches.md`). Change
...
is not byte-stable across an open (see the fixture pitfalls in `patches.md`).
