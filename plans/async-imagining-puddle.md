...
- This collapses `/*pfg-vX.Y*/` (extension.js signature, v1.x's inline F.1 placement) AND `/*pfg-v2.0:ext*/` (v2.0's first-line-prepend signature) to the same sentinel, and similarly collapses `pfg-vX.Y` / `pfg-v1.6` / `pfg-v2.0` text-mentions inside Patch K's UI bookend (verified via grep: `the archived prebuilt's apply.py` contains 1 mention of `pfg-v1.6` and 3 of `pfg-vX.Y`) to the same sentinel.
