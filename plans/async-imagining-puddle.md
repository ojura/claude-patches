...
`claude-patches` currently maintains 12 extension-side patches (A-L) for Claude Code (current `v1.x` line), packaged as per-version prebuilt `apply.py` scripts with literal `(old, new)` text splices, byte-stability validation (`util/build-prebuilt.py`), `.bak` discipline, and idempotency via embedded `/*pfg-vX.Y*/` signature (`version.py`). Linux-x64 only; three target files (`extension.js`, `webview/index.js`, `webview/index.css`) per `util/build-prebuilt.py`.
...
- **Patch S (streaming-thinking restoration)**, CLI-side, repo-authored. End-to-end owner of the streaming-thinking display path - both the reducer-side writer (the per-`thinking_delta` accumulator that pushes progressive updates into the React state setter that holds streamingThinking) and the renderer-side prop chain (the live and transcript renderers receive streamingThinking, the row component destructures it, and the message-list memo folds the streaming thinking entries alongside the existing streaming tool uses). **Supersedes connoisseur's CON-F (`patchThinkingStreaming`) entirely.** Reason: connoisseur's matcher for the renderer-side seed returns zero matches on every 2.1.16x bundle tested empirically; the literal connoisseur expects hasn't existed since at least the early 2.1.16x line. Result on a connoisseur-only binary: connoisseur's writer fires partially (a subset of its sub-replacements survive anchor drift) and writes accumulate into the React state, but no renderer reads them - finalized thinking visible (via Anthropic's pristine one-shot assistant-message writer), progressive streaming invisible. Patch S replaces both halves with a discovery-based patcher: it finds the per-release minified identifiers by structural pattern and emits the writer body + renderer prop wiring against them, failing loudly if any anchor returns 0 or >1 matches. Patch S is `owner="repo"` (lives in `util/patch_streaming_thinking.py`), `required=True`, `smoke_observable=True`. CON-F drops to `required=False, smoke_observable=False` and is documented as "writer-only partial, superseded by S" - kept in the registry only as a fallback if Patch S anchors all 0-match on some future bundle, but the smoke must check Patch S's progressive surface, not CON-F's outcome. **Patch S also owns the streaming-thinking instrumentation surface**: per-PID structured logs across the writer body, the React setter, and the renderer chain, attributing each event to the layer that produced it so a grader can distinguish progressive writes from finalized-message writes. The instrumentation is opt-in (`--instr` flag at synthesis time); production prebuilts ship without hooks. The script `util/patch_streaming_thinking.py` is the source of truth for sub-patch enumeration, anchor patterns, discovered names, and the current hook surface - see its module docstring and the changelog for the per-version specifics. The stress harness (`util/streaming_thinking_stress.md` + `util/check_streaming_log.py`) verifies the wired behavior end-to-end with a derivation-stressing knight problem; pass thresholds and current per-Sonnet-on-current-build numbers live there, not in this plan.
...
2. **Unified target model** that covers extension files + CLI payloads under one `apply.py` without breaking idempotency. Current model has signature-in-extension.js only; v2.0 needs per-target signatures and a state machine that handles missing-backup, version-mismatch, standalone-only CLI installs, and multi-install loop (a behavior change from v1.x's error-on-multi-install). Falsified by Slice 2.
...
- **Patchset version**: bump from the current `v1.x` line to `pfg-v2.0`. Single signature family covers all four surfaces; per-target signature probes (§7).
- **Patches M + N + S are new CLI-side patches.** M = subagent UI message drop. N = #59844 Option 1 (drop `!getIsNonInteractiveSession()` from the showThinkingSummaries branch). S = streaming-thinking restoration (writer + renderer end-to-end); supersedes connoisseur's CON-F. Patch L stays as the existing `--thinking-display` extension.js splice (Option 2), retained as belt-and-braces for N. CON-A through CON-J are connoisseur's display tweaks (all CLI-side); O is our repo-owned welcome-badge patch (CLI-side) that supersedes connoisseur's CON-K.
...
`util/build-prebuilt.py` defines `targets` as three direct file paths (tuples of `(relpath, fullpath)`). v2.0 replaces this with a **typed Target model** split across two lifecycles:
...
Dispatch is now explicit (`kind == "text"` reads/splices/writes the file directly; `kind == "bun_js"` extracts the **entrypoint module's** JS from a bun binary, splices, repacks). No string-suffix sentinels, no relpath-as-behavior-switch. Adding a new target type (`bun_wasm`, `electron_asar`, ...) is a new enum member, not a new parsing rule.

**Scope of `bun_js`**: v2.0 patches only the entrypoint module of the bun bundle. The 2.1.150 binary has 5 modules total (entrypoint `cli.js`, plus `image-processor.js`, `image-processor.node`, `audio-capture.js`, `audio-capture.node`); only `cli.js` is patched. Addressing a non-entrypoint module (e.g., a future patch that needs `image-processor.js`) would require extending TargetSpec with an optional `bun_module_name: str | None` field defaulting to the entrypoint; deferred to future scope. If a v2.0+ implementer needs to patch a sibling module, this is the extension point, not a hidden invariant somewhere in bun_handler. The four v2.0 TargetSpecs:
...
2. **`template_after_format.replace("# __BUN_HANDLER_INLINE__", bun_handler_source.rstrip())`** second, splicing the bun handler source in as raw Python. The handler source can contain `{`, `}`, `"""`, `'''`, f-strings, dict/set literals, anything; `str.replace()` doesn't reinterpret them. **Line-ending tolerance**: the sentinel match deliberately omits a trailing `\n` and the source is rstripped because a Windows checkout with `core.autocrlf=true` rewrites the template's line endings to `\r\n`; a `"# __BUN_HANDLER_INLINE__\n"` match would find zero hits there, leaving the bun handler not inlined and apply.py crashing at first CLI target. Sentinel-without-newline matches regardless of line-ending style.
...
Connoisseur's hardened TS patcher (post Phase 0) is invoked **only at synthesis time** as a bootstrap subprocess. End-user `apply.py` contains only literal Python `str.replace` tuples, same shape as today, with the usual `.bak` + `count==1` + byte-stability rigor (mirrors the existing splice loop at `util/build-prebuilt.py`).
...
| CON-F | `patchThinkingStreaming` | thinking-streaming (multi-site) - **superseded by Patch S**; renderer-seed anchor (`hidePastThinking:!0,streamingThinking:VAR`) 0-matches on 2.1.16x. Kept `required=False, smoke_observable=False` as a fallback; Patch S owns both writer + renderer end-to-end. |
...
| Patch S (streaming-thinking restoration; supersedes connoisseur's `patchThinkingStreaming`) | repo-authored (`util/patch_streaming_thinking.py`) | discovery-based end-to-end restoration of streaming-thinking: writer side (4 anchored sub-patches inside c2H: stream_request_start reset, thinking content_block_start init, text content_block_start snapshot, thinking_delta progressive accumulator body) AND renderer prop chain (4 sub-patches: live-mode lxH props, transcript-mode lxH props, QyA destructure, MH useMemo fold). Owns the `--instr` instrumentation surface (9 hooks; opt-in). |
...
**Anchor freshness check for Patch O (load-bearing, do this BEFORE Slice 3 wires Patch O into PATCH_MODULES)**: the spec above describes appending to a settings `title:` template literal of the form `` `Claude Code v${...VERSION}` ``. This shape was inferred from connoisseur's `patchWelcomePatchedBadge` regex, but **the shape no longer exists in the current 2.1.150 bundle** (verified empirically: connoisseur's regex returns 0 matches against the extracted JS; total `Claude Code` mentions: 401, but the welcome-banner-with-version site has migrated). The actual welcome banner is now rendered as a TWO-REACT-ELEMENT pair:

```js
A9.createElement(k,{bold:!0},"Claude Code")," ",A9.createElement(k,{dimColor:!0},"v",VERSION_ID)
```

i.e., a bold `Claude Code` element, a literal `" "`, and a `dimColor` `v<VERSION>` element, all as siblings under a parent `k`. Both connoisseur's `patchWelcomePatchedBadge` (CON-K equivalent) and our Patch O target the obsolete template-literal shape, so both currently `anchor-failure` on day one. Before Slice 3 ships, the maintainer must (a) re-derive Patch O's anchor against the two-element form (recommend: locate the `createElement(k,{dimColor:!0},"v",<id>)` element via a unique-context regex around it and insert a sibling `createElement(k,{dimColor:!0}," pfg-v<patchset-version>")` element after it), (b) confirm the result renders correctly in the welcome banner, and (c) update Patch O's spec here + the §1 patch_ids table + Verification §13's expected output. If the implementer prefers to defer the re-derivation, the alternative is to mark Patch O as `required=False` until the new anchor lands (so anchor-failure routes to log+skip rather than blocking the build); this matches the same shape as Patch M's anchor freshness check in §4.

**PATCH_MODULES registry** (adopted from connoisseur, extended to cover everything): inside `util/build-prebuilt.py`'s bootstrap section, each of A through L (Slice 0 ports) + Patch M + Patch N + Patch S (streaming-thinking restoration) + CON-A through CON-J (connoisseur display tweaks) + O (repo-authored welcome badge) becomes a module with metadata:
...
class PatchModule:
    ...
    smoke_observable: bool                   # True: smoke() returns a real CLI-observable assertion; False: cosmetic-only stub (see §6 step 11c)
    apply: Callable                          # apply(content: str, ctx: PatchCtx) → ApplyResult; ctx carries (version, target_token, source_sha256, is_synthesis_time)
    smoke: Callable                          # smoke(binary_path: str) → (ok: bool, detail: str); cosmetic stubs return (True, "cosmetic-only, no CLI-observable assertion") per §6 step 11c
```
...
A-L + M/N + O + S have `owner="repo"` (live in `util/patch_modules/`; Patch S specifically in `util/patch_streaming_thinking.py` as a self-contained module - see Critical files); CON-A through CON-J have `owner="connoisseur"` (imported from the vendored TS patcher). `required`: A-L + M + N + O + S are `required=True`; CON-A through CON-J are `required=False` (including CON-F, the now-superseded thinking-streaming patch).

**Registry ordering produces the S/CON-F supersession with no special mechanism.** The orchestrator iterates patches in the order A-L, M, N, S, CON-A...CON-J, O (Patch S inserted ahead of the connoisseur block). When Patch S succeeds, its writer-body splice replaces the c2H thinking_delta case literal that CON-F's matcher anchors against. When CON-F's turn arrives, its `before` literal is no longer in the bundle, so connoisseur's matcher 0-matches and CON-F skips with `skipped_reason="not_present_in_this_version"`. No `supersedes` field on PatchModule, no dependency-graph logic in the orchestrator: ordering plus `required=False` plus the natural anchor-disappearance is sufficient. If Patch S 0-matches its anchors on some future bundle (a discover-stage failure), synthesis aborts at S's `anchor-failure` BEFORE reaching CON-F; the user gets a hard fail with S's discovery diagnostic, not a silent fall-through to CON-F's partial-writer-no-renderer outcome.

Patch S smoke-tests its own progressive-write surface via the W1 instrumentation hook (synthesis-time --instr only); CON-F's smoke returns the cosmetic-stub tautology since it can't independently prove streaming visibility on 2.1.16x.
...
- Steps:
  ...
  11b. **Strict-mode real-binary tool gates** (the control-flow proof must not silently degrade on a future bundle bump). Run `CLAUDE_NATIVE_BINARY=<patched_cli_abs_path> CLAUDE_PFG_STRICT_GATES=gate7 python3 util/test_bun_handler.py`. The strict flag promotes a SKIP on the listed gates to a FAIL: gate 7's "anchor not present in this build" outcome no longer exits silently; CI rejects the build until the maintainer either refreshes the anchor in `test_bun_handler.py` (the to-be-documented MAINTAINER.md responsibility; the v2.0 docs pass adds the section) or explicitly waives via `CLAUDE_PFG_STRICT_GATES_WAIVE=gate7`. Default behavior with no env var stays skip-permissive for local dev. Without this step, the control-flow proof would silently stop being exercised on the exact Anthropic bundle bump where the underlying assumption is most likely to drift. Failure sets `status=tool-gate-regression`; aggregator opens a `needs-human-review` issue with the gate's diagnostic output attached.
  11c. **Patch-level smoke tests** (broader than gate 7's architectural-assumption check; catches per-patch behavior regressions on each version). Each PATCH_MODULE declares a `smoke_observable: bool` flag (see §5 PatchModule dataclass) AND a `smoke(binary_path) -> (ok: bool, detail: str)` method. `smoke_observable=True` modules' smokes return a real CLI-observable assertion; `smoke_observable=False` cosmetic stubs return `(True, "cosmetic-only, no CLI-observable assertion")` uniformly. After step 11's binary smoke and 11b's strict-mode tool gates, the CI runner iterates every applied module from `status.json`'s `patches_applied` whose `smoke_observable=True` and calls its smoke against the patched CLI binary. CI lints that `smoke_observable == True` matches modules whose smoke does NOT return the cosmetic-stub tautology, so a future maintainer can't silently add a real assertion to a "cosmetic" patch's smoke without updating the flag. **Env-isolation discipline** (load-bearing for any `smoke_observable=True` smoke): each smoke runs in an isolated subprocess with `HOME=tempfile.mkdtemp()` so it cannot pollute the runner's `$HOME/.claude/settings.json`. Smokes needing custom Claude settings use the documented `--settings <file>` flag (per `claude --help`) rather than mutating HOME. Patch M's effective signal (subagent messages dropped on tab-switch) manifests ONLY in IDE-hosted interactive mode; a non-interactive `claude --print` cannot reproduce the bug, so Patch M's smoke MUST be marked `smoke_observable=False` for v2.0 (or upgraded to a future IDE-driven harness in a later milestone). Patch N's smoke writes a `settings.json` with `showThinkingSummaries=true` to a tempdir, invokes `claude --print --settings $TMP/settings.json`, and asserts thinking blocks appear in the JSONL. Smoke implementations should be lightweight (single CLI subprocess + output match, ideally < 5s) and assert an OBSERVABLE behavior change a user would notice. Examples: Patch M's smoke (once the §4 anchor is re-derived against current bytes) spawns a `--print` subprocess that triggers the subagent code path and asserts no message-drop in the JSONL; Patch N's smoke runs `claude --print` against a settings file with `showThinkingSummaries=true` and asserts thinking blocks appear; CON-I version-output checks that the patched marker shows. CON-* tweaks that aren't CLI-observable (CON-B/C diff colors, CON-H spinner tips, Patch O's settings `title:` site) return `(True, "cosmetic-only, no CLI-observable assertion")` so the iteration is uniform but the gap is logged for a future visual-test framework. Failure of any non-cosmetic smoke sets `status=patch-smoke-failure`. Asymmetry vs gate 7: gate 7 verifies the bytecode-execution-model assumption (one fixed test that catches a future bun runtime change); patch-level smokes verify each PATCH's behavior landed correctly on each version (per-version, per-patch, catches an anchor that matched a structurally-similar but functionally-different site).
  12. Upload as workflow artifact (`if: always()` so failed jobs still upload). Wrap each step in an error trap that writes `status.json` *before* the upload step runs; `if: always()` alone isn't enough because checkout-adjacent setup, OpenVSX fetch, npm fetch, unzip, or tool install can fail before our explicit status-writing logic runs. Pattern: `set -Eeuo pipefail; trap 'echo "{\"status\":\"infra\",\"failed_step\":\"$STEP\"}" > status.json' ERR` at the top of the run-block, with `STEP=<name>` set before each command. The `-E` (errtrace) flag is load-bearing: bash's default ERR trap does NOT fire on commands inside conditionals or pipelines, so without it a failed `curl -L $url | tar xf -` skips the trap; `-e` (errexit) makes the script exit on first failure; `-o pipefail` propagates pipeline failures; `-u` (nounset) catches unset-variable typos in `$STEP`. Aggregator treats missing artifact OR `status=infra` as an infrastructure failure (distinct from the synthesis-failure classes). Artifact contents: `apply.py` (self-contained, bun handler embedded), `metadata.txt`, byte-stability report, smoke-test log, `status.json`. Filename: `prebuilt-$VERSION-$PLATFORM.tar`.
  13. **Failure classification only** (build jobs are `contents: read`; they cannot dispatch workflows or open issues). On any failure, write the classification to `status.json` and exit. The aggregator reads `status.json` and routes accordingly. Status enum (used consistently in both producer and consumer):
      ...
      **`status.json` schema** (the contract between build jobs and the aggregator; required fields):
      ...
      ```
      The aggregator and `claude-recover.yml` both read this schema; `recovery_owner` + `patch_id` + `excerpt_artifact` are what make recovery routing and dedup deterministic. For non-anchor-failure statuses the patch-specific fields are omitted; `format-ambiguity` carries its own diagnostics payload instead (see the `format-ambiguity` enum entry). **`target_relpath` is platform-variant** (`...claude` on linux-*/darwin-arm64, `...claude.exe` on win32-*); the aggregator's anchor-failure dedup key is `(version, patch_id)`, NOT `target_relpath`, since the JS the patch anchors against is platform-invariant per the cross-platform-divergence gate and aggregating same-patch failures across platforms into one recovery dispatch is the documented behavior.
...
- **Syntax gate before commit**: split into a *tokenless setup* job and a *tokenful execution* job so upstream package scripts can't run with `CLAUDE_CODE_OAUTH_TOKEN` in env:
  ...
  - A-O candidates (every repo-owned patch) → `python3 -m py_compile <path>` plus a quick import-test (`python3 -c "from <module> import apply"`), where both `<path>` and `<module>` are derived from `repo_patch_module_path(patch_id)` and `repo_patch_file_path(patch_id)`. Python stdlib only; no package install needed in the token job.
...
- **Slice 0** (local, one-time): derives PATCH_MODULES matchers for A-L from the existing v1.x prebuilt's literal tuples. One whole-file pass = 1 Claude invocation.
...
|---|---|---|---|
| Slice 0 (A-L derivation, local) | A-L docs from `docs/patches.md` + v1.x splice tuples from the archived v1.x prebuilts (`prebuilt/archive/v1.7/`) + pristine/patched bundle excerpts | `util/patch_modules/patch_*.py` (per-patch one file each) | `py_compile`, import smoke, v1.x reproduction on every archived v1.x prebuilt |
...
| `claude-recover.yml` (A-O, repo-owned) | failed patch module + bundle excerpt + failure diagnostics | exactly one `util/patch_modules/patch_*.py` (computed via `repo_patch_file_path`) | diff gate, `py_compile`, import smoke, synthesis retry |
...
- **Reads** are *constrained but not isolated*. The workflow uses `actions/checkout` with `sparse-checkout` to fetch only the relevant subtree (`util/patch_modules/` for A-O repo-owned sites, `vendor/connoisseur/` for CON-* sites, `util/bun_handler.py` + `util/test_bun_handler.py` for `recovery_owner=format` sites) plus the bundle excerpt passed as input. Sparse checkout reduces the visible read surface but isn't a true sandbox; `util/claude_call.py` could in principle `git fetch` more files before invoking the API (though it doesn't). We do *not* claim a fully isolated read surface; we claim a documented one. Soft gate.
...
**Target discovery**: extend the existing `find_default_ext_dir()` (`util/build-prebuilt.py`). The new model treats each *patchable surface* (extension JS files + each CLI binary copy) as an independent target with its own state. Discovery sources:
...
2. **Standalone CLI** (cross-platform, in priority order):
   1. `CLAUDE_CODE_EXECPATH` env var if set (existing carve-out in `find_default_ext_dir()`).
   ...
   3. Conventional install locations as fallback:
      - Linux/macOS: `~/.local/bin/claude` symlinks to `~/.local/share/claude/versions/<VER>` (where `<VER>` IS the binary file itself, NOT a directory containing `claude`; the version directory is `~/.local/share/claude/versions/` and each binary inside is named by version with no `/claude` suffix)
...
**Per-target idempotency**: previously the `/*pfg-vX.Y*/` signature lived only in `extension.js`. For v2.0, each patched surface carries its own signature stamp at synthesis time:
...
Per-target state-machine check (replaces v1.x's `has_current_sig = SIGNATURE in head`):
...
**Multi-install behavior change vs v1.x**: v1.x's multi-install guard in `find_default_ext_dir()` errors out when multiple installs of the same extension version are found, requiring the caller (SKILL.md's outer install loop that runs `python3 "$PREBUILT" "$EXT"` per install) to invoke apply.py once per install with an explicit path. V2.0 apply.py supports both modes via an explicit invariant:
...
Document this contract in CHANGELOG (breaking-change for direct-invocation users who relied on the v1.x "error on multi-install" behavior) and in apply.py's `--help` output.
...
- `node --check` on extension.js + extracted CLI JS: **optional + warn-don't-fail at apply time**. Three failure modes the current `except FileNotFoundError` in `util/build-prebuilt.py`'s apply template doesn't distinguish:
  1. **Node not on PATH** (FileNotFoundError): warn + skip. Preserves v1.x behavior.
  ...
  Implementation pattern (replaces v1.x's `sys.exit`-on-parse-fail behavior in `util/build-prebuilt.py`'s apply template):
  ```python
  try:
...
- **macOS (Mach-O, `darwin-arm64`)**: in-place edits invalidate Apple's existing signature. Without re-signing, Gatekeeper blocks the patched binary on first launch with a "cannot verify developer" dialog. Mitigation: bun handler runs `codesign -f -s - <bin>` post-splice on Darwin to apply an ad-hoc signature (same one-liner connoisseur uses at `vendor/connoisseur/.github/workflows/patch-claude.yml:168`; `-s -` is the ad-hoc sentinel meaning "no identity, no certificate", `-f` forces re-signing). `codesign` ships in `/usr/bin/codesign` as part of macOS itself (not Xcode CLT), so no PATH check needed. End-user UX (best case): first launch shows the Gatekeeper warning once, user clicks "Open Anyway" in System Settings, future launches proceed silently. **macOS 15+ caveat**: ad-hoc codesign does not satisfy Apple's notarization requirement (which the as-shipped Anthropic binary may have had). On Sequoia and later, Gatekeeper's "Open Anyway" override is per-binary-identity, scoped per binary, and the override path is deeper in System Settings (Privacy & Security, "Allow Anyway"). Every extension update changes the binary's identity (we splice + re-ad-hoc-sign on each apply), so macOS 15+ users may need to re-approve after each extension auto-update. Worth being honest about in user-facing docs and apply.py's post-apply summary on darwin runs. Validate per §13 step 10. A real Apple Developer ID cert would skip the first-launch prompt via notarization, but requires the maintainer's identity-tied private key in repo secrets (operationally hazardous and revocable by Apple if abused), so ad-hoc is the right choice for a public repo.
...
- **Legacy (v1.x and earlier)**: `prebuilt/<VER>/apply.py` (no platform subdir). Rendered as a `linux-x64`-only row.
...
workflow_run_id: <int>
patches_required: A,B,C,D,E,F,G,H,I,J,K,L,M,N,O,S
patches_applied: A,B,C,D,E,F,G,H,I,J,K,L,M,N,CON-A,CON-B,CON-C,CON-D,CON-E,CON-G,CON-H,CON-I,CON-J,O,S
```
...
- **Required** (every repo-owned patch: A through L, M, N, O, S): 0 anchor matches is `anchor-failure`. The build FAILS, no metadata.txt is written, no prebuilt ships, `claude-recover.yml` fires. A required patch can NEVER be silently absent from `patches_applied`, because its absence aborts the build before metadata exists.
...
So "correctly absent" applies ONLY to optional patches. The invariant: `patches_applied ⊇ {all required patch IDs}` always holds for any published prebuilt (any required miss would have failed the build). metadata.txt also records `patches_required` (the static required set) so a reader can confirm the superset relationship held. **Source**: `patches_required` is written by `build-prebuilt.py` at metadata.txt emit time from `sorted({m.id for m in PATCH_MODULES if m.required})`; it is NOT in the synthesized apply.py (apply.py only knows about the actual splices that got produced, not the static required set). The PATCH_MODULES registry is in scope at synthesis time, so this is a synthesis-side computation.
...
2. **Step 0c (prebuilt lookup), blocker if missed**: `skill/SKILL.md`'s Step 0c hardcodes `[ -f "$REPO_ROOT/prebuilt/$VER/apply.py" ]` and the curl URL `prebuilt/$VER/apply.py`. V2.0 changes the layout to `prebuilt/$VER/$PLATFORM/apply.py`. Without an update, the skill silently fails to find any v2.0 prebuilt and falls through to manual mode. Step 0c must:
   ...
   - Fall back to the legacy path `$REPO_ROOT/prebuilt/$VER/apply.py` (for pre-v2.0 prebuilts that stay in place; see "v1.x prebuilt coexistence" below).
   - Update the curl fallback URL to include `$PLATFORM` in the new path; the legacy URL stays as the second fallback.

**v1.x prebuilt coexistence**: pre-v2.0 prebuilts at `prebuilt/<VER>/apply.py` (no platform subdir) remain in place; the skill's dual-path lookup finds them as the legacy fallback. Users on extension versions that only ever shipped under v1.x keep getting v1.x patches; users on extension versions with a v2.0 prebuilt get v2.0. The pfg-version-mismatch guard at Step 0c still applies: if the user's local clone has bumped `version.py` to 2.0 but the only prebuilt for their extension version is v1.x, the guard treats it as "no prebuilt" and falls through to manual rather than mixing pfg versions.
...
- `util/build-prebuilt.py`: replace the v1.x `targets = [(relpath, fullpath), ...]` tuple list with a list of `TargetSpec` dataclass instances (build-time model; see §1). Adopt PATCH_MODULES registry in bootstrap section, iterating per TargetSpec to produce per-target nested splices. **Also note**: the script currently derives version from directory names ending in `-linux-x64` only; refactor to accept arbitrary platform suffixes from OpenVSX's `downloads` map.
- `util/build-prebuilt.py` (`PREBUILT_TEMPLATE`): per-platform dispatch (delegates to inlined `bun_handler.py`; see §1), `--list-patches` / `--dry-run` / `--cli-path` / `--force` flags, smoke tests, per-target idempotency. **Also revisit the template docstring**: it was de-hardcoded to a generic "All patches applied" (it previously said "Patches A through K"); for v2.0, decide whether to keep the generic form or enumerate "A through O (repo) plus CON-A through CON-J (connoisseur)". **v2.0 placeholder enumeration** (full set for `.format()`, plus the sentinel for `.replace()`):
  - `{version}`: extension version (e.g., `2.1.149`).
  - `{platform}`: OpenVSX platform identifier (e.g., `linux-x64`). Replaces the hardcoded `-linux-x64` literal + glob in the current template.
  - `{patchset_version}`: pfg version (e.g., `2.0`). Currently v1.x derives this via regex from SIGNATURE in the template; v2.0 makes it explicit so per-target sigs can be assembled without re-parsing.
  ...
  - `{targets_repr}`: the `TARGETS = [TargetPayload(...), ...]` list literal, with each entry colocating `relpath`, `kind`, `signature`, and `splices` (per §1's TargetPayload dataclass). Replaces v1.x's flat-tuple targets list AND the proposed-but-discarded TARGET_SIGS dict. Used by apply.py's runtime loop (load/splice/write per target) and runtime detection (`target.signature in head[:64]`). The TargetPayload class definition is emitted into apply.py just above the TARGETS list so it's importable / inspectable.
...
- `util/build-prebuilt.py` and `:336` (broken-archive guardrail Case (b)): update the `out_path` computation from `prebuilt/<VER>/apply.py` (legacy) to `prebuilt/<VER>/<platform>/apply.py` (v2.0 layout). Case (b) catches "maintainer re-synthesized without bumping pfg version" by byte-comparing against the currently-published apply.py; the comparison needs the same path structure as the new outputs. Case (a) (vs. `prebuilt/archive/broken/*/<version>/apply.py`) goes inoperative under v2.0; accept and document. See §11.
...
- `version.py` regex: update `(\d+(?:\.\d+)?)` to `(\d+(?:\.\d+)*)` so it accepts arbitrary-segment versions (e.g., `2.0.1` for regression-bump per §11's "Post-publish regression handling"). The current regex matches max one `.X` segment; against `2.0.1` it returns `2.0`, breaking the bump workflow silently. Without this update, the maintainer's `2.0 → 2.0.1` bump would not propagate to PATCHSET_VERSION and Case (b) byte-equality guard would correctly refuse to re-publish, but with a confusing "no functional change" message rather than a clear "version regex didn't catch your bump" message.
- `util/patch_streaming_thinking.py`: **BUILT** - repo-authored, self-contained, discovery-based patcher that owns the streaming-thinking display chain end-to-end (Patch S). Replaces the role of connoisseur's CON-F entirely on 2.1.16x and later, where CON-F's renderer-side seed anchor returns 0 matches and its writer-side anchors only partially fire. Per-release minified identifiers are discovered by structural pattern at synthesis time; every lookup asserts a unique match and fails loudly on drift, rather than versioning the patcher per release. The patcher owns two halves: a renderer-side prop chain (propagating streamingThinking from the live and transcript render callsites through the row component's destructure into the message-list memo) and a reducer-side writer absorbed from connoisseur's matching function (initialize on thinking-block start, snapshot on text-block start, accumulate per delta, reset on stream-request-start). The `--instr` flag adds an opt-in per-PID logging surface across the writer body, the React setter, and the renderer chain, attributing each event to its source layer so a grader can tell progressive writes from finalized-message writes. See the script's module docstring for the current sub-patch enumeration, hook surface, discovered-name list, and the rationale behind each anchor - those are per-release artifacts and live with the code, not in this plan. The stress harness (`util/streaming_thinking_stress.md` + `util/check_streaming_log.py`) verifies end-to-end wiring with an interleaved derivation-stressing knight problem; current pass thresholds (Sonnet-calibrated) live there. Connoisseur's other display patches (CON-A/B/C/D/G/H/I/J) are independent and stack cleanly with Patch S; CON-E and CON-J currently anchor-drift on 2.1.16x and stay `required=False`. **Integration into PATCH_MODULES**: at v2.0 integration, Patch S's `apply(content, ctx)` calls into `patch_streaming_thinking.py` as a library function (the script's `main()` is the maintainer-facing entry point; the discovery + splice logic is what `apply` reuses). `smoke_observable=True`; the smoke runs a fresh non-interactive `claude` call with a short thinking-eliciting prompt in an isolated tempdir-HOME, asserts the resulting JSONL has a thinking content block, AND (if `--instr` was used at synthesis) asserts the patcher's progressive-writer log line appeared at least once during the run.
...
- `util/extract_splices.py`: extended with `expected_count` emission, `WidenCollision`/`_resolve_collision` two-pass for identical-context multi-site edits, `_verify_splices` round-trip check, and `errors='surrogateescape'` on every decode/encode site including the collision-marker path. 30/30 checks in `util/test_splices.py`. The `Splice` dataclass in PREBUILT_TEMPLATE **must** include `expected_count: int = 1` and the generated apply.py **must** use the symmetric surrogateescape contract from `apply_splices_to_bytes` (the reference apply in test_splices.py decodes target file bytes with `errors='surrogateescape'`, runs the splice loop, encodes back with the same option). Now wired into `build-prebuilt.py`: the template keeps `expected_count` and the generated apply loop gates on it.
- `util/sync-version-mentions.py`: extend to generate README status table from `prebuilt/` tree. `SYNC_TARGETS` (`util/sync-version-mentions.py`) stays README.md-only. **Also fix the pfg-version regex**: `r"pfg-v\d+(?:\.\d+)?"` only matches one optional `.X` segment, so a `2.0.1` regression-bump would rewrite mentions to `2.0` (matching `2.0` and dropping `.1`). Update to `r"pfg-v\d+(?:\.\d+)*"` (same fix as `version.py`); the two regexes must stay in lockstep since both parse the same SKILL.md version line.
- `version.py`: regex update (see the dedicated entry above); plus refresh the docstring's patchset-version example to `2.0` when bumping the version. SSOT-from-SKILL.md model otherwise unchanged.
- `skill/SKILL.md`:
  - Bump the `**Patchset version**` line to `2.0`.
  ...
  - **Step 0c (prebuilt lookup), blocker if missed**: change hardcoded `prebuilt/$VER/apply.py` (local + curl URL) to dual-path lookup (platform-aware first, legacy fallback). See §11.
  ...
  - **New patch steps**: insert as Steps 15, 16, 17 (Patch M CLI-side, Patch N CLI-side #59844 Option 1, CON-A through CON-J + Patch O manual reference), renumbering the existing Step 15 "summary to the user" to Step 18 so the summary stays last in the workflow.
...
- `MAINTAINER.md`: document `CLAUDE_CODE_OAUTH_TOKEN` setup + subtree pull ritual + (a) the **ambiguous-modules_len fail-closed policy and recovery path**: if a future Anthropic build trips `BunFormatError("ambiguous module table layout")`, two options apply: (i) the maintainer adds a stronger discriminator to `_disambiguate_module_struct_size` and updates the test fixture manually; or (ii) the automated `claude-recover.yml` format-recovery dispatch (`status=format-ambiguity` → `recovery_owner="format"`, see §6 step 13) proposes a discriminator under the `util/bun_handler.py` + `util/test_bun_handler.py` allow-list with the rich diagnostics the failing build job emits. Either way, bypassing the guard via tweakcc's "default to 52 on ambiguity" approach (per `connoisseur-patches/scripts/vendored-elf-native.ts` and the bundled tweakcc dist) is NOT on the table; that is the silent-mis-pick path our refusal explicitly avoids, and our policy is stricter than tweakcc's on purpose. (b) the **gate 7 anchor refresh contract**: the anchor `}.BUILD_REF_NAME){return""}` in `test_bun_handler.py` is build-specific. On any Anthropic bundle bump that moves the jS-body shape, the maintainer must refresh the anchor and commit before the release synthesis runs. CI synthesis is strict-mode (§6 step 11b): a SKIPped gate 7 fails the build. The waive escape valve (`CLAUDE_PFG_STRICT_GATES_WAIVE=gate7`) is reserved for the rare case where the maintainer has confirmed the architectural invariant another way; using it without that confirmation regresses the spike's load-bearing proof to chat-history archaeology again.
- `docs/patches.md`: add Patch L section (currently absent, Patch L shipped in v1.x prebuilts but never got a docs section), plus Patch M + Patch N sections + CON-A through CON-J + Patch O reference table. Cross-reference Patch N from Patch L's new section (N is Option 1, preferred; L is Option 2, belt-and-braces).
...
- `CHANGELOG.md`: v2.0 entry, including the **breaking behavior change**: apply.py now loops over all discovered installs in one invocation (v1.x errored on multi-install). SKILL.md's outer per-install bash loop becomes redundant for v2.0+ prebuilts but stays for legacy v1.x compatibility.
...
0a. **Audit `docs/patches.md` against live v1.x splice tuples (maintainer-side, not fully automated).** **Prereq**: Patch L's section is currently absent from `docs/patches.md` (Patch L shipped in v1.x prebuilts but never got a docs section, per the Critical files entry below). Write that section FIRST, before the audit, by deriving it from the live L tuple in the archived prebuilt's `apply.py` (the `--thinking-display summarized` extension.js splice that implements #59844 Option 2) plus the docs/debugging.md v1.4 / 2.1.126 case study; this is cheap and removes the "audit a non-existent doc" ambiguity below. Then walk through each patch in A-L manually: open `docs/patches.md`'s section for that patch (which describes the change as prose + a `diff`-style block + placeholders like `<VAR>` for minified names) and cross-reference against the actual `(old, new)` tuple in the archived prebuilt's `apply.py`. The comparison isn't tuple-vs-tuple (the doc format doesn't carry Python tuples); it's "does the prose accurately describe what the splice actually does, and does the diff block match the live transformation modulo minifier drift between archived versions". Tooling can help (extract the doc's diff block, sed-normalize placeholders, byte-compare against the apply.py tuple's old/new spans), but the audit is fundamentally a maintainer-eyeball pass with sed assistance, not a one-button check. Any drift between docs and live tuples must be reconciled BEFORE step 1; otherwise Claude derives matchers against stale documentation and produces wrong matchers that the v1.x reproduction gate later catches as wholesale failures (and the maintainer wastes a Claude call). Update `docs/patches.md` to match live tuples (the live tuples are ground truth), not the other way around. **Watch for live-tuple bugs**: if a live splice is itself buggy (e.g., F.1's currently-shipped `(old, new)` contains `await 0(K.filePath, K.fileSize)` where `pE0` was minified-name-truncated to `0` during a prior splice edit; if that code path is reachable it throws `TypeError: 0 is not a function`), fix the behavior in the new PATCH_MODULE rather than faithfully reproducing the typo. A corrected tuple may fail byte-equality against the v1.x prebuilt, which is the right outcome: the normalization gate (re.sub PFG_SENTINEL) collapses signature-string differences but does NOT absolve obvious behavior bugs. The audit gates Slice 0.
1. Read A, B, C, D, E, H, I, J, K, L from `docs/patches.md` and the archived v1.x prebuilts (`prebuilt/archive/v1.7/`) for ground-truth splice tuples. F and G are already structurally implemented in `skill/apply-patch-fg.py`: reuse directly (port the regex logic into PATCH_MODULES entries for F and G; don't re-derive).
2. **One whole-file Claude derivation pass (local mode)** (same shape as Phase 0 §3): run `claude` CLI locally with a single invocation producing a full Python module of PATCH_MODULES entries covering the 10 remaining patches (A, B, C, D, E, H, I, J, K, L) with structural matchers, derived from the literal tuples. **Feed every archived v1.x prebuilt's splices** as input, not just the newest; minifier-output drift between the oldest and newest forces the derived regexes to be the most-tolerant. Downstream gates (py_compile, byte-exact v1.x reproduction across all 5 archived prebuilts) catch hallucinated output. No GitHub workflow involved; the CI plumbing for `claude-recover.yml`/`claude-harden.yml` is introduced in Slice 4.
3. **Preserve F.1's inline signature placement during the port.** The v1.x `/*pfg-vX.Y*/` signature lives inside F.1's replacement code at `updateSessionState(z,V,K){/*pfg-vX.Y*/let _p=...` (verifiable via grep in the archived prebuilt's `apply.py`). Slice 0's port keeps this inline placement exactly as `skill/apply-patch-fg.py` does, so byte-equality against the v1.x prebuilts holds after pfg-version normalization. **The first-line-prepend signature scheme described in §7 is introduced in Slice 2, not Slice 0**.
...
4. Local sanity check: against a pristine install of any archived v1.x version, run the new PATCH_MODULES registry. The resulting bundle should byte-equal what the v1.x prebuilt's apply.py produces, after the pfg-version normalization documented in the Slice 0 gate.

**Slice 0 gate**: PATCH_MODULES synthesis must reproduce every existing v1.x prebuilt's output. Test against all archived v1.x prebuilts under `prebuilt/archive/v1.7/` (the `archive/broken/` builds are excluded by definition). For each, run the new PATCH_MODULES against the pristine vsix for that version, then byte-compare the resulting bundle against the v1.x prebuilt's apply.py output **after a one-way normalization step on BOTH sides**:

- Apply `re.sub(r'pfg-v\d+(\.\d+)?(:\w+)?', 'PFG_SENTINEL', text)` to both the v1.x prebuilt's bundle output and the v2.0 PATCH_MODULES bundle output before comparing.
- This collapses `/*pfg-vX.Y*/` (extension.js signature, v1.x's inline F.1 placement) AND `/*pfg-v2.0:ext*/` (v2.0's first-line-prepend signature) to the same sentinel, and similarly collapses any `pfg-vN` text-mention inside Patch K's UI bookend (the archived prebuilt carries several, from the patchset versions in play when it was built) to the same sentinel.
- The direction is "normalize both, then compare" rather than "rewrite v1.x→v2.0 on one side", which means the gate passes regardless of whether the v2.0 port interpolates the current `PATCHSET_VERSION` into Patch K's bookend (recommended, for consistency with Patch O's interpolation) or freezes the literal.

Note: Slice 0's port preserves F.1's inline signature placement (per step 3 above); the first-line-prepend scheme is introduced in Slice 2 as a separate migration. So during Slice 0 the v2.0 PATCH_MODULES bundle still has the inline signature (just text-rewritten to `/*pfg-v2.0*/`), which normalization collapses to the same sentinel as v1.x's inline `/*pfg-vX.Y*/`.
...
4. Local synthesis + apply across the matrix of install configurations:
   ...
   - Previously-patched at v1.x, upgrading to v2.0
   - Missing `.bak`s
...
7. Version bump: `skill/SKILL.md`'s patchset-version line to `2.0`; cascade via `util/sync-version-mentions.py`.
...
- Bun bundle format: ~1-2 breaks per year (the load-bearing format is unstable enough that tweakcc maintains a vendored ELF fallback; our handler fails closed where tweakcc silently defaults to 52, per the 36-vs-52 ambiguity policy in §1).
...
2. **Local synthesis** on freshly-installed extension+CLI at v2.1.149/linux-x64: run maintainer bootstrap + `python3 util/build-prebuilt.py`. Verify byte-stability for all 4 splice targets (existing check at `util/build-prebuilt.py` + new bun-binary check).
