...
A-L + M/N + O + S have `owner="repo"` (live in `util/patch_modules/`; Patch S specifically in `util/patch_streaming_thinking.py` as a self-contained module - see Critical files); CON-A through CON-J have `owner="connoisseur"` (imported from the vendored TS patcher). `required`: A-L + M + N + O + S are `required=True`; CON-A through CON-J are `required=False`. **CON-F is also `required=False`** despite being a "thinking" patch: it's the superseded fallback for Patch S; if both are present in the registry and Patch S succeeds, CON-F is skipped with `skipped_reason="superseded_by_S"`. Patch S smoke-tests its own progressive-write surface via the W1 instrumentation hook (synthesis-time --instr only); CON-F's smoke returns the cosmetic-stub tautology since it can't independently prove streaming visibility on 2.1.16x.
...
- **Required** (every repo-owned patch: A through L, M, N, O): 0 anchor matches is `anchor-failure`. The build FAILS, no metadata.txt is written, no prebuilt ships, `claude-recover.yml` fires. A required patch can NEVER be silently absent from `patches_applied`, because its absence aborts the build before metadata exists.
