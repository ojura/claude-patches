...
for target in TARGETS:
    material = load_target(target)               # text read OR bun_js extract
    ...
    write_target(target, material)               # text write OR bun_js repack + atomic rename
```
...
**Recovery routing uses the explicit `owner` field, NOT prefix-matching**: claude-recover.yml routes a failed patch to `util/patch_modules/patch_<id>.py` if `owner=="repo"`; `vendor/connoisseur/patch-claude-display.ts` if `owner=="connoisseur"`; `util/bun_handler.py` + `util/test_bun_handler.py` if `owner=="format"` (the bun-format recovery dispatch from a `status=format-ambiguity` failure, distinct from patch-anchor recovery; see the `format-ambiguity` entry in §6 step 13's enum). This replaces the earlier fragile `startswith("CON-")` prefix check (which would have mis-routed O, and would silently mis-route any future repo-owned patch that happened to start with those letters). The owner field is the single source of truth.
  ...
  13. **Failure classification only** (build jobs are `contents: read`; they cannot dispatch workflows or open issues). On any failure, write the classification to `status.json` and exit. The aggregator reads `status.json` and routes accordingly. Status enum (used consistently in both producer and consumer):
      ...
      **`status.json` schema** (the contract between build jobs and the aggregator; required fields):
      ```json
      {
        ...
        "owner": "repo",                        // "repo" | "connoisseur"; recovery routing reads THIS, not a prefix heuristic
      ...
      ```
      The aggregator and `claude-recover.yml` both read this schema; `owner` + `patch_id` + `excerpt_artifact` are what make recovery routing and dedup deterministic. For non-anchor-failure statuses the patch-specific fields are omitted.
...
- **Recovery scope routed by failing patch's `owner` field, enforced by post-action diff gate**:
  - Owner is read from the failing patch's `PatchModule.owner` field (§5), NOT inferred by prefix-matching the ID. The failure's `status.json` carries `owner` (see status.json schema below), so the routing is a lookup, not a string heuristic:
    ```python
    owner = failure["owner"]   # "repo" | "connoisseur", recorded at synthesis-failure time
    if owner not in ("repo", "connoisseur"):
        raise ValueError(f"unknown owner {owner!r} for patch {failure['patch_id']!r}")
    ```
    This deliberately replaces the earlier `patch_id.startswith("CON-")` heuristic, which would mis-route **O** (repo-authored welcome badge, no CON- prefix but conceptually replaces connoisseur's CON-K) and any future repo-owned patch.
  - `owner == "connoisseur"` → allow-list is `vendor/connoisseur/patch-claude-display.ts`.
  - `owner == "repo"` → allow-list is the filesystem path returned by `repo_patch_file_path(patch_id)` (see helper below).
  - `owner == "format"` → allow-list is `util/bun_handler.py` + `util/test_bun_handler.py`. Dispatched from `status=format-ambiguity` (NOT `anchor-failure`); see the `format-ambiguity` enum entry in §6 step 13 for the rich-diagnostics payload requirement, the "propose a stronger discriminator OR keep fail-closed" prompt shape, and the explicit "do not mimic tweakcc's default-to-52" constraint.
