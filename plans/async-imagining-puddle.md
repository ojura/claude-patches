...
|---|---|---|---|
| Slice 0 (A-L derivation, local) | A-L docs from `docs/patches.md` + v1.x splice tuples from all 5 archived prebuilts (2.1.142, .143, .145, .146, .148) + pristine/patched bundle excerpts | `util/patch_modules/patch_*.py` (per-patch one file each) | `py_compile`, import smoke, v1.x reproduction on all 5 archived prebuilts |
...
1. Read A, B, C, D, E, H, I, J, K, L from `docs/patches.md` and the v1.x prebuilts (2.1.142, .143, .145, .146, .148) for ground-truth splice tuples. F and G are already structurally implemented in `skill/apply-patch-fg.py`: reuse directly (port the regex logic into PATCH_MODULES entries for F and G; don't re-derive).
