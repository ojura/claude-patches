---
name: patch-claude
description: Reapply Juraj's eleven local patches to all anthropic.claude-code IDE extension installs. Discovers every install across VS Code, Antigravity, Cursor, VSCodium via CLAUDE_CODE_EXECPATH + glob. Use when the user says "the extension updated, reapply patches" or similar. Backs up files, reapplies all eleven patches, verifies each one.
---

# Reapply anthropic.claude-code extension patches

**Patchset version**: `1.11`

Eleven patches live out-of-tree and need to be reapplied every time a bundled
...
- `PATCHES_APPLIED` → skill is complete. Tell the user to reload their IDE(s).
  ...
  *current* patchset version (`python3 "$REPO_ROOT/version.py"`). If
  ...
  `.bak`, apply ALL patches (A-K) manually per Steps 3-13 below +
...
- If a `.bak` from a prior version already exists, leave it. The current
  ...
  reinstall.
- **Retired (v1.10): former Patch L** (force `--thinking-display
  summarized` on the IDE-spawned CLI). Upstream fixed the underlying bug in
  extension `2.1.176`: `spawnClaude` now reads `showThinkingSummaries` and
  passes `--thinking-display summarized` itself (see
  [#59844](https://github.com/anthropics/claude-code/issues/59844)). Keep
  `"showThinkingSummaries": true` in `~/.claude/settings.json` to get thinking
  summaries on Opus 4.7+; no patch needed.
